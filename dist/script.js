...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var showUpSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_1__["default"]({
    ...
    // class в котором размещены все слайдеры блока
    prev: '.showup__prev',
    next: '.showup__next',
  ...
  player.init();
  new _modules_difference__WEBPACK_IMPORTED_MODULE_3__["default"]('.officerold', '.officernew', '.officer__card-item').init(); // альтернативный метод вызова, без создания переменной
});
...
function () {
  function Difference(oldOfficer, newOfficer, items) {
    ...
    // oldOfficer - первый столбик, newOfficer - второй столбик, items - селектор (class)
  ...
  _createClass(Difference, [{
    ...
    value: function bindTriggers(container, items, counter) {
      // вместо закоментированного ниже, оптимизация кода
...
function () {
  function VideoPlayer(triggers, overlay) {
    ...
    // triggers - кнопка play, overlay - модальное окно
  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          if (document.querySelector('iframe#frame')) {
            // если документ имеет тег iframe с id frame, т.е, если плеер уже запущен
            _this.overlay.style.display = 'flex'; // показываем блок
          } else {
            var path = btn.getAttribute('data-url'); // id video указан в вверстке, как дата атрибут кнопки
    ...
    value: function bindClose() {
      ...
      this.close.addEventListener('click', function () {
        _this2.overlay.style.display = 'none'; // скрываем блок с видео

        _this2.player.stopVideo(); // stopVideo - метод с API, https://developers.google.com/youtube/iframe_api_reference?hl=ru 
    ...
    value: function createPlayer(url) {
      // создаем youtube player, код с https://developers.google.com/youtube/iframe_api_reference?hl=ru
      this.player = new YT.Player('frame', {
        // frame = class из index.html 
        height: '100%',
        width: '100%',
        videoId: "".concat(url) // уникальный id видео, которое на youtube
    ...
    key: "init",
    value: function init() {
      var tag = document.createElement('script'); // подключаем youtube player, код с https://developers.google.com/youtube/iframe_api_reference?hl=ru
...
function (_Slider) {
  ...
  // наследуется (имеет доступ ко всем свойствам и методам) от Слайдера
  function MainSlider(btns) {
    _classCallCheck(this, MainSlider);

    return _possibleConstructorReturn(this, _getPrototypeOf(MainSlider).call(this, btns)); // получаем свойства
  }
    ...
    value: function render() {
      ...
      this.btns.forEach(function (item) {
        item.addEventListener('click', function () {
          _this2.plusSlides(1); // 1 - только для первой страницы, там только одна кнопка

        });
        item.parentNode.previousElementSibling.addEventListener('click', function (e) {
          // обращаемся к лого (ссылка, вверхний левый угол) 
          e.preventDefault();
          _this2.slideIndex = 1;
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      if (!this.slides[0].closest('button')) {
        // closest - возвращает ближайший или самого себя
    ...
    value: function bindTriggers() {
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          // запускаем цикл с конца псевдомассива
...
var Slider = // можно сразу експортировать, чтобы не писать в конце. Наименование Class-ов пишется с большой буквы
